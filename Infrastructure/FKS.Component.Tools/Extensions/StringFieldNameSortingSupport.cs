...
namespace FKS.Component.Tools
{
    public static class StringFieldNameSortingSupport
    {
        ...
        #endregion
        public static IOrderedQueryable<TEntity> OrderBy<TEntity>(this IQueryable<TEntity> source, string fieldName, string fieldSort, string defaultFName) where TEntity : class
        {
            if (fieldName == null)
            {
                fieldName = defaultFName;
            }
            if (fieldSort == null)
            {
                fieldSort = "asc";
            }
            if (fieldSort == string.Empty || fieldSort.ToUpper() == "ASC")
            {
                return OrderBy(source, fieldName);
            }
            else
            {
                return OrderByDescending(source, fieldName);
            }
        }
