...
{
    partial class EquipManagerViewController
    {
        partial void SetQueryBuilder(Pagination pagination)
        {
            ...
            if (pagination.KeyValues != null)
            {
                ...
                {
                    //if (kv.Key.Equals("Result"))
                    //{
                    //    queryBuilder.Equals(s => s.Result, int.Parse(kv.Value));
                    //}
                    if (kv.Key.Equals("PositionInfo"))
                    {
                        queryBuilder.Equals(s => s.PositionInfo, int.Parse(kv.Value));
                    }
                    else if (kv.Key.Equals("PropertyInfo"))
                    {
            ...
            this.ViewQueryBuilderEquipment = queryBuilder;
        }

        partial void SetQueryBuilder2(string[] strs)
        {
            IQueryBuilder<EquipManager> queryBuilder = QueryBuilder.Create<EquipManager>();
            queryBuilder.In<EquipManager, string>(s => s.CollectionCode, strs);
            this.ViewQueryBuilderEquipManager = queryBuilder;
        }
        ...
        {
            var equip = this.SiteContract.EquipManagers.SingleOrDefault(m => m.CollectionCode.Equals(model.CollectionCode));
